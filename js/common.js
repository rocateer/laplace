...
document.addEventListener("DOMContentLoaded", function () {
  // 모든 팝업 버튼에 이벤트 리스너 추가
  document.querySelectorAll(".popup_area").forEach(button => {
      button.addEventListener("click", function () {
          const url = this.dataset.url; // data-url 값 가져오기
          const width = parseInt(this.dataset.width, 10); // data-width 값 가져오기
          const height = parseInt(this.dataset.height, 10); // data-height 값 가져오기
...
// 팝업을 중앙에 띄우는 함수
function openCenteredPopup(url, popupWidth, popupHeight) {
  const screenWidth = window.innerWidth;
  const screenHeight = window.innerHeight;
  const left = (window.screenX || window.screenLeft) + (screenWidth - popupWidth) / 2;
  const top = (window.screenY || window.screenTop) + (screenHeight - popupHeight) / 2;
...
document.addEventListener("DOMContentLoaded", () => {
  const menuWraps = document.querySelectorAll(".more_wrap");

  menuWraps.forEach((menuWrap) => {
    const button = menuWrap.querySelector(".btn_more");
    const menu = menuWrap.querySelector(".more_layer");

    button.addEventListener("click", () => {
      // 다른 모든 메뉴 닫기
      document.querySelectorAll(".more_layer").forEach((m) => {
